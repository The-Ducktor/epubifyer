...
test("Ensure epubcheck is installed", async () => {
  try {
    const proc = Bun.spawn(["epubcheck", "--version"]);
    await proc.exited;
    const exitCode = await proc.exitCode;
    expect(exitCode).toBe(0);
  } catch (error) {
    console.error("epubcheck is not installed or not found in PATH");
    throw error;
  }
});
...
test("Validate EPUB with epubcheck", async () => {
  try {
    ...
    if (exitCode === null) {
      console.warn("Warning: epubcheck may not be installed or accessible in PATH");
