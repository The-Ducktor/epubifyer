...
class Epub {
      ...
      if (this.isurl(chosenUrl || "")) {
        try {
          ...
          if (chosenUrl) {
            ...
            const ext = compress ? "jpeg" : chosenUrl.split(".").pop()?.split("?")[0] || "img";
  ...
  async addCover(source: string): Promise<string> {
    ...
    if (source.startsWith("data:")) {
      ...
      try {
        ...
        throw new Error(`Failed to fetch cover image (${source}): ${(error as Error).message}`);
      }
    } else {
      ...
      try {
        ...
        throw new Error(`Failed to read cover image file: ${(error as Error).message}`);
      }
    }
  ...
  async addChapter(
    ...
    let chapterId = id;
    if (!chapterId) {
      chapterId = `chapter_${++this.uniqueIdCount}`;
    }
    ...
    if (Array.isArray(html)) {
      ...
      if (
        ...
        if (raw) {
          const sorted = (html as Array<[number, string]>).slice().sort((a, b) => a[0] - b[0]);
    ...
    for (let i = 0; i < partHtmls.length; ++i) {
      const partId = partHtmls.length === 1 ? chapterId : `${chapterId}_part${i + 1}`;
...
${partHtmls[i]}
  ...
  private addToParentNavPoint(
    ...
    for (const navPoint of navPoints) {
      ...
      if (navPoint.children && this.addToParentNavPoint(navPoint.children, parentId, newNavPoint)) {
        return true;
      }
    }
    return false;
